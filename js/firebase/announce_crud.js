...
function getBriefAnnounce(col, val){
    const refAlert = firestore.collection('alert');
    const box_title = document.getElementById('not-title');
    const box_con = document.getElementById('not-msg');
    
    
    refAlert.where(col, "==", val).orderBy("date", "desc").get().then((snaps)=>{
        var count = 0;
        snaps.forEach((row)=>{
            
            box_con.innerHTML = row.data().content;
            box_title.innerHTML = row.data().title;
        });
    });
}
...
function getAnnounceFire(id, cat){
    ...
    refAlert.doc(id).get().then((result)=>{
        ...
        message.innerHTML = result.data().content;
